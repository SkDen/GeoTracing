...
class OpenClKernelLoader:
    ...
    def _calculate_batch_size(self, total_rays: int, dtype: type) -> int:
        """Вычисление размера батча с учетом ограничений памяти"""
        item_size = np.dtype(dtype).itemsize
        # Учитываем размер всех буферов: начальные условия + point_status + point_flag
        memory_per_ray = 8 * item_size + 8 * item_size + 4  # 4 байта для int32
        max_batch_size = self.max_alloc_size // memory_per_ray
        batch_size = min(total_rays, 4000, max_batch_size)
    ...
    def _run_kernel_ray_tracing(self, program: cl.Program, metric_type: MetricType, 
        ...
        # Добавляем специфичные для метрики параметры
        if metric_type == MetricType.MINKOWSKI:
            # Для метрики Минковского не нужны дополнительные параметры
            pass
        elif metric_type == MetricType.SCHWARZSCHILD:
            ...
            kernel_args.append(np.float64(L/M))  # Угловой момент на единицу массы
        ...
        # Определяем оптимальный размер рабочей группы
        ...
        if current_batch_size <= max_work_group_size:
            ...
            # Используем максимально возможный размер рабочей группы
