...
from ImageConstruction import PanoramicSkySphereRenderer
...
if __name__ == "__main__":
    ...
    camera = Camera(width=W, height=H, focus=1.0, aspect_ratio_inv=False, metric=current_metric)
    # 4-вектор позиции камеры в сферический координатах
    camera_pos = Vector4(t=0, x=2.24, y=np.pi/2, z=np.pi/2, vtype=VectorType.COORDINATES, dtype=np.float64)
    # Задаем поворот камеры
    camera.set_direction(theta=0, phi=np.pi)
    ...
    renderer = PanoramicSkySphereRenderer(point, 
                                          flag, 
    ...
    # Построение изображения с учетом интенсивности
    intensity_image = renderer.render_with_intensity(intensity_factor=1.5)

    # Визуализация результатов
    renderer.visualize(rendered_image, save_path="rendered_sky.png", title="Rendered Sky Sphere")
    renderer.visualize(intensity_image, save_path="intensity_sky.png", title="Sky Sphere with Intensity")

    # Сохранение изображений
    renderer.save_image(rendered_image, "final_render.png")
    renderer.save_image(intensity_image, "final_render_intensity.png")

    # Получение статистики
    stats = renderer.get_statistics()
    print("Статистика трассировки лучей:")
    print(f"Всего лучей: {stats['total_rays']}")
    for status, count in stats['status_counts'].items():
        percentage = stats['status_percentages'][status.name]
        print(f"{status.name}: {count} лучей ({percentage:.2f}%)")


    # np.savetxt('GeodesicEquations\\full_array.txt', point, delimiter='\t', encoding='utf-8', fmt='%.3f')
    # np.savetxt('GeodesicEquations\\flags.txt', point_counts, delimiter='\t', encoding='utf-8', fmt='%.3f')
